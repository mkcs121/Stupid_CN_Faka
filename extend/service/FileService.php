...
class FileService
{
    ...
    public static function getFileUrl($filename, $storage = null)
    {
        ...
        switch (empty($storage) ? sysconf('storage_type') : $storage) {
            ...
            case 'qq':
                return self::getBaseUriCos() . $filename;
        }
        return false;
    }
     ...
     * @return string
     */
    public static function getBaseUriCos()
    {
        ...
        return $http . sysconf('storage_qq_domain') . '/';
    }
        ...
        switch (empty($storage) ? sysconf('storage_type') : $storage) {
            ...
            case 'qq':
                ...
                require 'vendor/cos-php-sdk-v5/vendor/autoload.php';
                    ...
                    [
                    'region' => sysconf('storage_qq_region'),
                    'credentials'=> [
                        'secretId'    => sysconf('storage_qq_secret_id'),
                        'secretKey' => sysconf('storage_oss_secret_key')]
                    ]);
                        return $cosClient->getObject(array(
                            
                            'Bucket' => sysconf('storage_qq_bucket'),
    ...
    public static function qiniu($filename, $content)
    {
		...
		list($ret, $err) = $uploadMgr->putFile($token, $filename, $content);
        if ($err !== null) {
     ...
     * @return array|null
     */
    public static function tencent($filename, $content)
    {
        try {
            require 'vendor/cos-php-sdk-v5/vendor/autoload.php';
        ...
        if(sysconf('storage_qq_is_https')==1){
            ...
            return ['file' => $filename, 'hash' => $result['RequestId'], 'key' => $filename, 'url' => $http.$result['Location']];
        } catch (\Exception $e) {
            Log::error('腾讯云Cos文件上传失败, ' . var_export($e, true));
