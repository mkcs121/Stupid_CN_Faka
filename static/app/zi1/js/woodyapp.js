						...
						if (typeof res.data.pop == "string") {
							...
							if(res.data.isMobile){
								layer.open({
									...
									content: '<div style="padding: 0 15px 0 15px;border-radius:10px 10px 10px 10px;">' +
										'<div style="color:#000000;text-align:center;font-size:18px;padding:20px 0 10px 0;font-weight:bold;word-wrap:break-word">商家公告</div>' +
										'<div style="padding:0 0 10px 0;font-size:14px;color:#727272;text-align:center;word-wrap:break-word">' +
										res.data.pop +
										'</div>' +
										'<div style="padding:0 0 15px 0;font-size:14px;color:#000000;text-align:center;">有任何商品使用问题请联系卖家QQ:<a href="//wpa.qq.com/msgrd?v=1&uin=' +
										res.data.qq +
										'&site=mjfaka.com&menu=yes" target="_blank" style="color: red;font-weight:bold ">' +
										res.data.qq +
										'</a></div>' +
										'<div style="height: 45px;"><button class=" layui-layer-close layui-layer-close2" href="javascript:;" style="border-radius:10px 10px 10px 10px;width: 70%;left: 15%;line-height: 26px;padding: 0 18px;background-color: #2A62FF;color: #fff;white-space: nowrap;text-align: center;font-size: 14px;border: none;cursor: pointer;">我已阅读</button></div>' +
										'</div>'
								});
							}else{
								layer.open({
...
// 	console.log($('.goods_box_leng_xz .signal-price').html());
// 	var m =
// 	$('#pay-price').html(($('.goods_box_leng_xz .signal-price').html() * $("#quantity").val()).toFixed(2));
	goodschk();
}
...
function selectcate(obj) {
	...
	if (cateid > 0) {
		let sected_goods_id = $("#sected_goods_id").val();
		if(sected_goods_id) {
			sected_goods_id = sected_goods_id;
		} else {
			sected_goods_id = 0;
		}
		$.post("/ajax/getgoodlist", {
			...
			if (data == "ok") {
				...
				for (var i = 0; i < data.length; i++) {
					html += '<div class="goods_one goods_box_leng" data-name="' + data[i].name + '" data-val="' + data[i].id +
						'" data-price="' + data[i].price + '" onclick="selectgood(this)">'
					html += '<div>' + data[i].name + '</div>'
					html += '<div style="">￥<span class="signal-price">' + data[i].price + '</span></div>'
					html += '<div style="display: none;"></div>'
					html += '<div><span><!-- <p style="width: 0%;background:#fff;"></p> --></span> '

					// html += '<span>剩余' + data[i].total + '件</span></div>'
					html += '<span>'+data[i].stockStr+'</span></div>'
					html += '<img class="shop-img" style="display:none" src="/static/img/xiadui.29ad1d0.png">'
					html += '</div>'
				}
				$('#shop-list').html(html);

				selectgood($(".goods_one")[0]);
			}
		});
	} else {
		$("#loading").hide();
...
function selectgood(obj) {
	$(obj).addClass('goods_box_leng_xz').siblings().removeClass("goods_box_leng_xz");
	var goodid = $(obj).attr("data-val");
    $("#goodid").val(goodid);
	$('#pay-price').html($(obj).attr("data-price")*1);
	//  * $("#quantity").val()
	$('#check-name').html($(obj).attr("data-name"))
	$(obj).find('.shop-img').css({
		display: 'block'
	})
	$(obj).siblings().find('.shop-img').css({
		display: 'none'
	})
...
function submit() {
	...
	var quantity = $.trim($("#quantity").val() || $("[name=quantity]").val());
	...
	var str = "";
	if(str.match(contact)){
		alert("联系方式过于简单，请重新输入！");
		$("[name=contact]").focus();
		return false;
	}
	$('#submit').addClass('notclickn');
	setTimeout("$('#submit').removeClass('notclickn')",5000);
